...
if (process.argv.slice(2).join('') === '-v') {
  ...
  console.log('create-node-package version ' + pkg.version);
  try {
    const cwd = process.cwd();
    const npPkg = resolve('node_modules/create-node-package/package.json');
    const npVersion = JSON.parse(readFileSync(npPkg, 'utf-8')).version;
    console.log('    create-node-package version ' + npVersion);
  } catch (e) {
  }
